...
/**
 * Global actions which contain the clear global state actions which can
 * be dispatch directly to the root reducer
 */
import { GlobalActions } from 'app/actions';

import { ReducerError } from 'app/models';
 ...
 */
const reducers = {
  application: fromSample.reducer,
};
...
export function reducer(state: any, action: any) {
  if (action.type === GlobalActions.actionTypes.CLEAR_GLOBAL_STATE) {
    // Clear all states besides router
    const { router } = state;
    state = { router };
  }

  if (action.type === GlobalActions.actionTypes.SET_GLOBAL_STATE) {
